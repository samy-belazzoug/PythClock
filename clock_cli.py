...
def clock(start, alarm, message):
    '''Allow user to launch a clock with an alarm from commands in the terminal.'''
    hour, minute, second, format_clock, am = start
    enabled_alarm, hour_alarm, minute_alarm, second_alarm, format_alarm, am_alarm = alarm
    if enabled_alarm == 0: # No alarm enabled
        if format_clock == 12 or format_clock == 0: # 12H Format
            if am == 0: # AM
                ...
                sleep(3)
                clk = clock_backend.Clock(hours=hour, minutes=minute, seconds=second, format24=False ,pm=False)
        elif format_clock == 24 or format_clock == 1: # 24H Format
            ...
            sleep(3)
            clk = clock_backend.Clock(hours=hour, minutes=minute, seconds=second, format24=True ,pm=False)
        clock_backend.clock_validity(clk) # Checks if hh mm ss are valid
        logging.info('User mentionned a correct clock.')
        return clock_backend.time_terminal_displaying(time=clk,alarm=False)

    elif enabled_alarm == 1: # No alarm enabled
        if format_alarm == format_clock: # Time format is the same for both clock and alarm
            if format_clock == 12: # 12H clock
                if am == 0: # AM clock
                    clk = clock_backend.Clock(hours=hour, minutes=minute, seconds=second, format24=False ,pm=False)
                elif am == 1: # PM Clock
                    clk = clock_backend.Clock(hours=hour, minutes=minute, seconds=second, format24=False ,pm=True)
                ...
                clk = clock_backend.Clock(hours=hour, minutes=minute, seconds=second, format24=True ,pm=False)
                clk_alarm = clock_backend.Clock(hours=hour_alarm, minutes=minute_alarm, seconds=second_alarm, format24=True ,pm=False)
            if clock_backend.clock_validity(clk): # If clock is valid
                if clock_backend.clock_validity(clk_alarm): # If alarm is also valid
        ...
        else: # Time format are different between clock and alarm
...
@cli.command("chrono")
def chrono():
    '''Allow user to launch a stopwatch from terminal with commands'''
